...
# 原始数据获取方式（AJST-Data 补充说明）

各事件目录下的 `OriginalFormats/` 原本保存了从期刊网页抓取的 HTML 页面快照。**为遵守版权，这些 HTML 快照不随本仓库分发，已全部移除**；个别目录中同时存放的非 HTML 原始文件（如论文 PDF 附表转换的 txt、OpenSNPhotometry.csv 等）仍保留。

如需核对原始数据：每个事件目录下的 `readme.md` / `readme.yml` 记录了数据出处（文献引用及来源链接），本目录的标准化数据文件（`*_Optical*.txt`、`*_Radio*.txt`、`*_Spectra*.txt` 等）即由这些已公开文献数据按本文件描述的格式转换而来。请根据各事件 readme 中的引用，从原始文献（期刊 DOI 页面）或 GRBSN webtool 项目自行获取原始数据。
